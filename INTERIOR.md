...
- **Window bands and galleries** at the pod end (r 1.42-1.74): vertical panes in the STL's own slot (below); the slot floor (+0.0325) is a real surface over the concourse flanks there, so the space behind the panes is an observation gallery on each flank, reached by a stair/lift core at each end from the concourse. The galleries exist because the STL puts a floor and a window there, not for their own sake.
