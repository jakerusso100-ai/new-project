...
- **Spine skylight** (v2.6: own tinted, reflective glass material with edge frames and a transverse mullion every 0.03 R laid on the ray-cast STL surface (`arm_section.top_surface`); the STL's thin spine fin is the ridge, so it reads as glazing rather than an opening): STL arm triangles within 0.07 R of the spine ridge and above +0.095 R render as glazing so the concourse is lit from above. Design decision on the owner's request (placement C): the STL has a raised spine there and the Printables render a lighter centre strip; frame #23 shows the arms lit; no frame shows the arm roof.
- **Light band** at the pod end (r 1.42-1.74): opaque lit bars standing in the STL's shoulder slot (#23 lit strips). No glazing and no gallery level behind them (owner, 2026-09-15; the earlier window panes and galleries were removed with the rest of the arm glass).
...
## v2.9 (2026-09-15): arms de-glazed, dome lattice on the STL ridges, keel column

Owner's call: the show's arms are solid, so every piece of arm glazing is gone (spine skylight and its frames, slot window panes and mullions, the window galleries). The slot keeps opaque lit bars (#23). The dock piers of v2.8 were removed. The dome lattice now sits on the STL's own six raised ridges (measured: az 0/60/120/180/240/300 on the main dome and on each pod, `dome_profile`, `pod_profile`), with the ring purlins on the measured panel profile; the old eight-rib ellipsoid lattice was neither on the ridges nor on the surface, which is what made it look off-centre. A translucent additive energy column now runs between the keel fins.
