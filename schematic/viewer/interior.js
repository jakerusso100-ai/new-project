...
DB.nodes.filter(n=>n.id.startsWith('ARM-')).forEach(n=>{ const pts=n.geometry.points; const a=Math.atan2(pts[0][1],pts[0][0]); const ca=Math.cos(a), sa=Math.sin(a); const r0=Math.hypot(pts[0][0],pts[0][1]), r1=Math.hypot(pts[2][0],pts[2][1]); const len=r1-r0;
  ...
  const slope=35*Math.PI/180; for(const sg of [1,-1]) for(let k=0;k<12;k++){ const rr_=r0+len*(k+0.5)/12; const u=sg*(0.235+0.0025*Math.sin(slope)); const [x,y]=[ca*rr_-sa*u, sa*rr_+ca*u]; const m=new THREE.Mesh(new THREE.BoxGeometry(0.045,0.018,0.003),winMat); m.position.set(x,y,0.03+0.0025*Math.cos(slope)); m.rotation.z=a; m.rotateX(sg*slope); cat.glass.add(tag(m,n.id,'flank window panel (#23)')); }
});
...
const HIDE=['DOME-P','DOME-P-HUB','DOME-S1','DOME-S2','DOME-MB','DOME-S1-HUB','DOME-S2-HUB','DOME-MB-HUB','LM-CORE-WATER','LM-CENTRAL-PLAZA','CIRC-RING-1','CIRC-RING-2','CIRC-RING-3','CIRC-RADIALS','CIRC-HOVER-BAND','CIRC-TRAM-PLAZA','CIRC-CORE-WALKWAYS'].concat(LM.map(n=>n.id)).concat(MESH_B64?['ARM-S1','ARM-S2','ARM-MB','SHELL-DRUM','SHELL-PLATES','SHELL-UNDERHUB','SHELL-LOWER-BODY','SHELL-UNDERSIDE','KEEL-FIN-1','KEEL-FIN-2','KEEL-FIN-3','KEEL-FIN-4','KEEL-FIN-5']:[]); // with the STL embedded, the mesh is the shell: no block-out duplicates
